...
docker stop ${id}
docker rm ${id}
docker rmi acmegrocery/battery:v2
echo *INFO* building a new NodeJS container image
docker build -t acmegrocery/battery:v2 .
sudo docker push acmegrocery/battery:v2

exit 0

echo *INFO* running our new image and exposing service on port 2020
docker run -p 2020:8080 -d acmegrocery/battery:v2

...
id=`docker ps -a|grep -i analysis|awk '{print $1}'`
docker stop ${id}
docker rm ${id}
docker rmi acmegrocery/analysis:v1
echo *INFO* building a new NodeJS container image
docker build -t acmegrocery/analysis:v1 .
sudo docker push acmegrocery/analysis:v1
